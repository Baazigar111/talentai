...
export default function CandidatesPage() {
    ...
    useEffect(() => {
        jobsAPI.getAll().then(r => setJobs(r.data));
    }, []);
    ...
    return (
        <div style={{ maxWidth: '960px' }} className="animate-fade">
          ...
          // AI RANKING ENGINE
                ...
                <h1 style={{ fontFamily: 'Epilogue', fontWeight: 900, fontSize: '36px', letterSpacing: '-1.5px' }}>
                    Candidates
                </h1>
            </div>
            ...
            <div className="card" style={{ padding: '20px', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '8px' }}>
                    ...
                    {jobs.length === 0 && (
                        <p style={{ fontSize: '13px', color: '#999' }}>No jobs yet. <a href="/jobs" style={{ color: 'var(--blue)', fontWeight: 700 }}>Create one →</a></p>
                    )}
                </div>
            </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                                            <div style={{
                                                width: 40, height: 40, background: 'var(--black)', borderRadius: 'var(--radius)',
                                                border: 'var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                                                fontFamily: 'Epilogue', fontWeight: 900, fontSize: '12px', color: 'var(--yellow)', flexShrink: 0,
                                            }}>
                                                {(r.name || 'NA').split(' ').map((n: string) => n[0]).join('').slice(0, 2)}
                                            </div>
                                            <div>
                                                <div style={{ fontFamily: 'Epilogue', fontWeight: 700, fontSize: '16px' }}>{r.name}</div>
                                                <div style={{ fontFamily: 'JetBrains Mono', fontSize: '11px', color: '#888' }}>{r.email}</div>
                                            </div>
                                        </div>
                                            ...
                                            {[
                                                ...
                                                <div key={label}>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                                                        <span style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', color: '#999', textTransform: 'uppercase' }}>{label}</span>
                                                        <span style={{ fontFamily: 'JetBrains Mono', fontSize: '9px', fontWeight: 700 }}>{val}%</span>
                                                    </div>
                                                    <div style={{ height: '6px', background: '#eee', border: '1px solid #ddd', borderRadius: '2px', overflow: 'hidden' }}>
                                                        <div style={{ height: '100%', width: `${val}%`, background: getScoreColor(val), transition: 'width 0.5s ease' }} />
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div style={{ textAlign: 'center', marginLeft: '20px', flexShrink: 0 }}>
                                        <div style={{
                                            ...
                                            <div style={{ fontFamily: 'Epilogue', fontWeight: 900, fontSize: '22px', letterSpacing: '-1px', lineHeight: 1 }}>
                                                {r.ats_score}
                                            </div>
                                            <div style={{ fontFamily: 'JetBrains Mono', fontSize: '8px', opacity: 0.7, marginTop: '2px' }}>ATS SCORE</div>
                                        </div>
                                        <button onClick={() => deleteCandidate(r.candidate_id)}
                                            className="btn btn-white" style={{ fontSize: '10px', marginTop: '8px', padding: '4px 8px' }}>
            ...
            {!loading && ranked.length === 0 && selectedJob && (
                <div className="card" style={{ padding: '40px', textAlign: 'center' }}>
                    <div style={{ fontFamily: 'Epilogue', fontWeight: 900, fontSize: '20px', marginBottom: '8px' }}>No candidates found.</div>
